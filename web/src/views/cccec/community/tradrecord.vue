        ...
        <row>
            <card>
                <div class="sreach">
                    <div class="sreach-page">
                        ...
                        <div class="sreach-box"><span>商品名称:</span>
                            <i-input v-model="goodsName" placeholder="请输入收货人..."></i-input>
                        </div>
                        ...
                        <div class="sreach-box">
                            <span>类型：</span>
                             <i-select  v-model="ordertypelist"  clearable=""   filterable="" style="width:100px" >
                    ...
                    <div class="sreach-page">
                         <div class="sreach-box sreach-box2" style="text-align: right;">
                            <i-button type="info" icon="ios-search" @click="gettradrecord(1)">搜索</i-button> 
                        </div>
                ...
                <div class="page-box" style="text-align:right;margin-top:15px;">
                    <page :total="pageCount" :page-size="pagesizes" :current="pageNumber" show-elevator="" show-total="" @on-change="gettradrecord"></page>
                </div>
            </card>
        </row>
    </div>
...
<script>
    ...
    import {
        closeorder,bindingcourierandorder,orderupdate,getcouriercompany,gettradrecord, orderupdate2,updatanewstatus,verificationNotice,orderrefundAmount,getmatchingArea4,getmatchingStage,updatamatchingArea4,getareabox,operatorupdateapi,getoperatorlist,sendmessagelog,getmatchingoperName,getoperatordata,updataAddress,updataorderArea4,updateconfirmoreder,getmerchantlist,imgurl,
    }
    from "./../../api/api.js";
    export default {
        name: "extension",
        data() {
            return {
                ...
                }, 
                {
                    ...
                    align: 'left',
                       render: (h, params) => {
                        return h('div', [h('p',params.row.order_no)]);
                    },
                },
                {
                    title: '商品图片',
                    key: 'coinTrade.goodsImage',
                    width:100,
                    align: 'center',
                    render: (h, params) => {
                        return h('div', {
                          attrs: {
                            ...
                            h('img', params.row['coinTrade.goodsImage']!=null?{
                              props: {
                                type: 'primary',
                                size: 'small'
                              },
                              attrs: {
                                src:imgurl+params.row['coinTrade.goodsImage'], style: 'width:30px;height: 30px;border-radius: 2px;'
                              },
                              style: {},
                            }:''),
                          ]);
                ...
                // },
                {
                    ...
                    render: (h, params) => {
                        return h('div', [h('div', params.row.orderType =='1' ? '普通' : params.row.orderType == '2'? '升级会员' : params.row.orderType =='3' ? '购买积分' : params.row.orderType == '4' ? '充值' : params.row.orderType == '5' ? 'OSO' : params.row.orderType == '6'?'认领': params.row.orderType == '7'?'已认领': params.row.orderType == '8'?'出让':'')]);
                    },
                },
                {
                    title: '订单状态',
        ...
        mounted() {
            ...
            if (sessionStorage.getItem("orderpeoplelists")) {
                ...
                this.gettradrecord(this.pageNumber,this.pageSizes);
            }else{
                gettradrecord('', '', '',this.ordertype,this.orderstatus,this.needDateFrom,this.needDateTo).then(response => {
        ...
        methods: {
            ...
            gettradrecord(val,pagesizenum) {
                ...
                if((this.needDateFrom!=''&&this.needDateTo=='')||(this.needDateFrom==''&&this.needDateTo!='')){
                    ...
                    gettradrecord(val,this.orderIds,this.goodsName,this.ordertype,this.orderstatus,this.needDateFrom,this.needDateTo).then(response => {
            ...
            updatestatus(id,status){
                ...
                updatanewstatus(id,status).then(response => {
                        ...
                        if(response.success){
                            ...
                            zhethis.gettradrecord(zhethis.pageNumber,zhethis.pageSizes);
                        }else{
            ...
            getstatus(status_index,status_index2){        
                if(status_index == 0){
                    this.orderstatuslist = ['0','1','2','3','4','5','7'];
                    console.log('status :'+ this.orderstatus);
                    this.gettradrecord(null, null, null,null,null,null,null,null,this.orderstatus,null,null, null,null,null, null,null, null, null,null,null, null,null,null);
