<template>
    <div>
        <row>
            <card>
                <p slot="title">
                    <icon type="ios-list"></icon>提成管理</p>
                <div class="sreach">
...
<script>
    ...
    import {
        getnpeoplemember,updatepeoplelist,updatpeoplememberid,addBalance,addPoint,updatemaxbuy
    }
    from "../../api/api.js";
    export default {
        name: "extension",
            ...
            return {
                ...
                tableColumns: [{
                    ...
                    render: (h, params) => {
                        return h('a', {
                          attrs: {
                            href: '#/ccces/operation/balancepage?id=' + params.row.id
                          },
                        }, [h('span', params.row.balance==null||params.row.balance==""?'0':params.row.balance)]);
                    }
                } , {
                    title: '库存积分',
                    key: 'point',
                    ...
                    render: (h, params) => {
                        return h('a', {
                          attrs: {
                            href: '#/ccces/operation/pointpage?id=' + params.row.id
                          },
                        }, [h('span', parseInt(parseFloat(params.row.point)*1000)/1000)]);
                    }
                } , {
                    title: '收益珠宝',
                    key: 'incomeCoin',
                    ...
                    render: (h, params) => {
                        return h('a', {
                          attrs: {
                            href: '#/ccces/operation/coinpage?id=' + params.row.id + '&coinType=1'
                          },
                        }, [h('span', parseInt(parseFloat(params.row.incomeCoin)*1000)/1000)]);
                    }
                } , {
                    title: '消费珠宝',
                    key: 'consumeCoin',
                    ...
                    render: (h, params) => {
                        return h('a', {
                          attrs: {
                            href: '#/ccces/operation/coinpage?id=' + params.row.id + '&coinType=2'
                          },
                        }, [h('span', parseInt(parseFloat(params.row.consumeCoin)*1000)/1000)]);
                    }
                }
        ...
        mounted() {
            ...
            if (sessionStorage.getItem("peoplelists")) {
                ...
                getnpeoplemember().then(response => {
        ...
        methods: {
            getpeoplememberList(val) {
                this.loading = true;
                var sorts = "";
                getnpeoplemember(val, this.pagesizes, sorts, this.peoplename, this.peoplemember, this.memberType,this.parentpeoplename).then(response => {
                    console.log(response);
                    this.tableData = response.data.list;;
                    this.pageCount = response.data.totalRow;
                    this.totalPages = response.data.totalPage;
