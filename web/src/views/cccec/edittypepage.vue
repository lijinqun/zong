<template>
    <div>
        ...
        <modal title="新增商品类型" v-model="visible" :loading="loading" @on-ok="insert" :closable="false">
...
<script>
    ...
    import {
    	getgoodtypelist,updategoodtype,insertgoodtype,delgoodtypeid
    }
        from "./../api/api.js";
    ...
    export default {
        ...
        mounted() {
            ...
            Bus.$off('delete');
            getgoodtypelist(this.pageNumber, this.pagesizes, this.id, this.name).then(response => {
            ...
            Bus.$on('delete', function (deleteId) {
            	...
            	delgoodtypeid(param).then(response => {
                        if (response.success) {
        ...
        methods: {
        	...
        	updateLists () {
        		if (this.selectedIdList !== null && this.selectedIdList !== undefined && this.selectedIdList !== '') {
                    ...
                    updategoodtype(param).then(response => {
                        if (response.success) {
            ...
            getTableList(val) {
            	this.pageNumber = val;
                getgoodtypelist(val, this.pagesizes, this.id, this.name).then(response => {
                    console.log(response);
                    this.tableData = response.data.list;
                    this.pageCount = response.data.totalRow;
                    this.totalPages = response.data.totalPage;
            ...
            insert() {
            	if(this.insertname){
	     			...
	     			insertgoodtype(param).then(response => {
