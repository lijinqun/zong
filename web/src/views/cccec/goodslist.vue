            ...
            <card>
                ...
                <div class="sreach">
                    <div class="sreach-page">
                        ...
                        <div class="sreach-box"><span>商品状态:</span>
                            <i-select v-model="isdel" clearable="" filterable="">
                                <i-option v-for="item in deldata" :value="item.id" :key="item.id">{{ item.name }}</i-option>
                            </i-select>
                        </div>
                    </div>
                    <div class="sreach-page">
...
//	import goodslistdatas from './goodslistdatas.js';
    ...
    import {
        getgoodslist,imgurl,updategoodlistsort,batchauditmerchandise,getcategoryIdsdata,deletegood,getmerchantlist
    }
        from "./../api/api.js";
    export default {
        name: "goodList",
...
//                      }
            ...
            getTableLists(val) {
                this.loading = true;
                setTimeout(() => {
                	 getgoodslist(val, this.pagesizes, this.goodname, this.id, this.typename, this.suppliername, this.isOnline, this.status,this.catid,this.parent2catid,this.parent1catid,this.isdel).then(response => {
                    console.log(response);
                    this.tableData = response.data.list;
                    this.pageCount = response.data.totalRow;
                    this.totalPages = response.data.totalPage;
