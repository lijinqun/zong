<template>
    <div>
        <row>
            <card>
                <p slot="title">
                    <icon type="ios-list"></icon>订单管理</p>
                <div class="sreach">
                    ...
                    <div class="sreach-page">
                       ...
                       <div class="sreach-box"><span>订单状态:</span>
                            <i-select v-model="orderstatuslist" multiple clearable="" filterable="">
                    ...
                    <div class="sreach-page">
                        <div class="sreach-box sreach-box2" style="text-align: right;">
                            ...
                            <i-button type="warning" @click="confirmoreder()">确认订单</i-button>
                            <i-button type="warning" @click="clickdelivery()">发货</i-button>
                            <i-button type="error" @click="closeoreder()">关闭订单</i-button>
                        </div>
...
<script>
    ...
    import {
        closeorder,bindingcourierandorder,orderupdate,getcouriercompany,getnorder, orderupdate2,updatanewstatus,verificationNotice,orderrefundAmount,getmatchingArea4,getmatchingStage,updatamatchingArea4,getareabox,operatorupdateapi,getoperatorlist,sendmessagelog,getmatchingoperName,getoperatordata,updataAddress,updataorderArea4,updateconfirmoreder,getmerchantlist
    }
    from "./../../api/api.js";
    export default {
        name: "extension",
...
//                    }
                ...
                {   
                    ...
                    align: 'left',
//                  render: (h, params) => {
//                      return h('div', [h('p',(params.row.consignee!=null? decodeURI(decodeURI(params.row.consignee)) : '')+(params.row["member.nickname"]!=null? " / "+decodeURI(decodeURI(params.row["member.nickname"])) : params.row["member.username"]!=null? " / "+decodeURI(decodeURI(params.row["member.username"])) : '')),h('p',params.row.mobile!=null? params.row.mobile : ''),h('p',(params.row.house!=null? decodeURI(decodeURI(params.row.house)) : "")+(params.row.numberBuildings!=null? " / "+decodeURI(decodeURI(params.row.numberBuildings)) : "")+(params.row.unit!=null&&params.row.unit!=""? " / "+decodeURI(decodeURI(params.row.unit))+"栋" : "")+(params.row.roomNo!=null&&params.row.roomNo!=""? " / "+decodeURI(decodeURI(params.row.roomNo))+"房" : ""))]);
//                  }
                },
                {
                    title: '电话',
                ...
                },
                {
                    ...
                    width:150,
                    render: (h, params) => {
                        return h('div', [h('div', params.row.create_date != null ? params.row.create_date+' ' : ''),h('div', params.row.payment_date != null ? params.row.payment_date+' ' : '')]);
                    },
                },
                {
                    title: '订单状态',
                    ...
                    render: (h, params) => {
                        ...
                        ? '待收货' : params.row.order_status == 4 ? '问题件' : params.row.order_status == 5 ? '已取消' : params.row.order_status == 7 ? '已确认' : ''),h('div', params.row.isClosed ==false ? '未关闭' : '已关闭')]);
                    },
                },
                {
                    title: '详情',
...
//              this.needDateTo=tomorrowneedDateFrom;
                ...
                getnorder('', '', '', '', '', '', '', '', this.orderstatus,'', '', '','', '', '','', '', '','', '', '','',this.ordertype).then(response => {
        ...
        methods: {
            ...
            getListData(val,pagesizenum) {
                ...
                if((this.needDateFrom!=''&&this.needDateTo=='')||(this.needDateFrom==''&&this.needDateTo!='')){
                    ...
                    getnorder(val, this.pagesizes, conditions, sorts, this.orderIds, this.usernames, this.userphones, this.innname, this.orderstatus, this.ordertypes, this.goodstypes,this.difference,this.needDateFrom,this.needDateTo,this.orderhousedata,this.pdbindingStage,this.orderarea2,this.orderarea3,this.orderarea4,this.pdbindingoperName,this.pdbindingsalesPromoter,this.selectOperName,this.ordertype,this.address, this.merchnatlist).then(response => {
