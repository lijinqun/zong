<template>
  <div class="page" style="overflow-y: auto;overflow-x: hidden">
                ...
                <van-list
                    ...
                    <li v-for="(index,key) in listData" :key="key" class="van-hairline--bottom" v-show="index.totalQuantity != index.tradedQuantity" @click="details(index.id)">
                        ...
                        <div class="number">
                            ...
                            <p><a :href="'#' + href + index.id">{{index.memberId == userInfo.id ? '关闭' : '我要出让'}}</a></p>
                        </div>
                    </li>
                </van-list>
            </ul>
...
export default {
  ...
  methods: {
        details(id){
            this.$router.push('/detailsSecond/' + id)  
        },
        onLoad() {
            let _this = this;
            setTimeout(() => {  
