<template>
  <div class="citydata" id='citydata'>
    <!--显示点击的是哪一个字母-->
    <div id="showLetter" class="showLetter"><span>A</span></div>
    <!--城市索引查询-->
    <div class="letter">
        <ul ref='rightclick' @click="gocity($event)">
    ...
    <!--城市列表-->
    <div class="container">
    <!-- <div class="container" @click="clickcity($event)"> -->
      <div class="city">
        <div class="city-list"><span class="city-letter">热门城市</span>
          <div class="hotcity">
            <p value="" @click="clickcity('全国','')">全国</p>
            <p  v-for="hot in hotcity" :value="hot.id" @click="clickcity(hot.area,hot.id)">{{hot.area}}</p>
          </div>
        </div>
        <div class="city-list" v-for="item in cityarray"><span class="city-letter" :id="item.firstPinyin+'1'">{{item.firstPinyin}}</span>
          <p :value="list.id" v-for="list in item.areas"  @click="clickcity(list.area,list.id)">{{list.area}}</p>
        </div>
...
export default {
  ...
  methods: {
    ...
    gocity(event,name){
      // document.documentElement.scrollTop=document.getElementById(event.srcElement.innerHTML+'1').offsetTop;
      // document.body.scrollTop=document.getElementById(event.srcElement.innerHTML+'1').offsetTop;

      document.getElementById('citydata').scrollTop=document.getElementById(event.srcElement.innerHTML+'1').offsetTop;
    ...
    clickcity(name,id){
      ...
      // this.$emit('clickcity', event.srcElement.innerHTML,event.srcElement)
