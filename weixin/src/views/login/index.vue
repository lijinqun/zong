<template>
  <div class="phone">
    ...
    <div class="data">
      ...
      <p class="registered"><router-link to="/registered">还没有账号？去注册</router-link><router-link to="/passwordlogin" class="password">账号密码登录</router-link></p>
...
<script>
import { appSendPhoneCode,appLogin} from "./../../api";
     ...
     if(this.tell){
       ...
       if(this.picked == 1){
         tell = '00852' + this.tell
          ...
          appLogin(this.tell,this.sms).then(response => {
            if(response.code==200){
              Toast.success('登录成功！');
